...
class AuthenticatedJourneyApiTests(TestCase):
    ...
    def test_list_journeys(self):
        ...
        journeys = Journey.objects.annotate(
            available_places=F("train__cargo_num")
    ...
    def test_filter_journeys_by_date(self):
        res = self.client.get(JOURNEY_URL, {"date": "2025-05-15"})
        journeys = Journey.objects.annotate(
            available_places=F("train__cargo_num")
            * F("train__places_in_cargo")
            - Count("tickets")
...
class AdminJourneyApiTests(TestCase):
    ...
    def test_create_journey(self):
        ...
        station_to = sample_station(
            name="Lviv Central", longitude=50.001, latitude=51.001
        )
        ...
        self.assertEqual(
            ...
            journey.departure_time.strftime("%Y-%m-%dT%H:%M:%S") + "Z",
        )
        self.assertEqual(
            payload["arrival_time"],
            journey.arrival_time.strftime("%Y-%m-%dT%H:%M:%S") + "Z",
        )
        self.assertEqual(
            list(journey.crew.values_list("id", flat=True)), payload["crew"]
        )
